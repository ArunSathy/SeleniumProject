...
chrome_options=webdriver.ChromeOptions() # Options : used to store the data of user, once the user logged in it won't ask again for the credentails. It will store the data in the below path as hidden
chrome_options.add_argument('user-data-dir=C:\\Users\\arhsathy\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\Whatsapp')

brw=webdriver.Chrome(service=driver,options=chrome_options)
brw.maximize_window()
